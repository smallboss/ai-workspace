...
const GameBoard = ({ bet6, chatParam: { chatOpen, setChatOpen } }: { bet6: string[], chatParam: { chatOpen: boolean, setChatOpen: Dispatch<SetStateAction<boolean>> } }) => {
    ...
    const placeBet: (i: number) => Promise<boolean> = async (i: number) => {
        try {
            ...
            if (status) {
                Game_Global_Vars.id[i] = game?.id || 1
            ...
            socket.on("message", async ({ message, update }: { message: string, update: number }) => {
                try {
                    if (message === "GAME_START") {
                        ...
                        setAviatorState(prev => {
                            const new_auto_count = prev.RemainedAutoPlayCount
                            new_auto_count[roundIdIndex] = new_auto_count[roundIdIndex] > 0 ? new_auto_count[roundIdIndex] - 1 : -1
                            return { ...prev, RemainedAutoPlayCount: new_auto_count }
                        })
                    }
        ...
        <div className='flex overflow-auto' style={{ height: "calc(100vh - 50px)" }}>
            ...
            <div className="flex flex-col gap-2 w-full bg-black p-2 text-white overflow-auto pb-0">
                ...
                <div className={`flex justify-center w-full relative`} style={{ height: pixiDimension.height }} ref={pixi_ref}>
                    <div className='flex flex-col absolute top-0 justify-center items-center'
                        style={{
                        ...
                        <div className='flex flex-col justify-center items-center'>
                            ...
                            {
                                aviatorState.game_anim_status === "ANIM_CRASHED" &&
                                <p className='text-[#e50539] font-bold text-[42px] leading-[42px] lg:text-[100px] lg:leading-[100px]'>{curPayout}x</p>
                            }
                        </div>
                    </div>
                    <PIXIComponent pixiDimension={pixiDimension} curPayout={curPayout} />
                </div>
                <div className="flex flex-col w-full" ref={footer_ref}>
                    ...
                    <div className={`grid grid-cols-1 gap-2 relative ${betButtonCount === 1 ? "sm:grid-cols-1 3xl:grid-cols-1" : "sm:grid-cols-2 3xl:grid-cols-2"}`}>
                        <button
                            onClick={() => {
                                    ...
                                    <div className='flex gap-2'>
                                        <div className='flex flex-col w-[100px] lg:w-[130px] 3xl:w-[260px] h-full'>
                                            <div className='flex justify-between items-center text-[10px] 3xl:text-xl w-full h-[27px] 3xl:h-[54px] bg-black rounded-full px-2' style={{ fontFamily: 'Roboto' }}>
                                                <button disabled={betPlaceStatus[i] === "success"} onClick={() => modifyBetValue(-10, i)} className='flex justify-center items-center w-[16px] h-[16px] 3xl:w-[36px] 3xl:h-[36px]  text-white rounded-full border-2 border-white/60'>
                                                    <div className='w-2/3 h-1/6 bg-white/60' />
                                                </button>
                                                <input disabled={betPlaceStatus[i] === "success"} onChange={(e) => handleBetValueChange(e, i)} value={betValue[i]} type="string" className='w-[30px] lg:w-[70px] 3xl:w-[150px] bg-black text-center text-[18px] font-bold 3xl:text-[26px]' />
                                                <button disabled={betPlaceStatus[i] === "success"} onClick={() => modifyBetValue(10, i)} className='flex relative justify-center items-center w-[16px] h-[16px] 3xl:w-[36px] 3xl:h-[36px] text-white rounded-full border-2 border-white/60'>
                                                    <div className='w-2/3 h-1/6 bg-white/60' />
                                                    <div className='absolute top-[2px] h-2/3 w-1/6 bg-white/60' />
                                                </button>
                                            </div>
                                            <div className='flex flex-wrap justify-between gap-1 w-full h-[40px] 3xl:h-[78px] text-white/60'>
                                                {
                                                    bet6.map((item, j) =>
                                                        <button disabled={betPlaceStatus[i] === "success"} key={j} onClick={() => _setBetValue(`${item}`, i)} className='flex justify-center items-center bg-[#171717] rounded-lg gap-[2px] w-[48px] lg:w-[60px] h-[17px] 3xl:w-20 3xl:h-9 text-[13px] 3xl:text-[26px]'>
                                                            {item}
                                                        </button>
                                                    )
                                                }
                                        ...
                                        </div>
                                        {
                                            betPlaceStatus[i] === "success" || pendingBet[i] ?
                                                allowedBet || pendingBet[i] ?
                                                    <div className='flex flex-col justify-center items-center text-xs w-[180px] lg:w-[210px] 3xl:w-[395px]'>
                                                    ...
                                                    <button onClick={() => handleCashOut(i, false)} className='flex flex-col w-[180px] lg:w-[210px] h-[72px] 3xl:w-[395px] 3xl:h-[142px] rounded-[14px] 3xl:rounded-[30px] bg-gradient-to-b from-[#E59407] to-[#412900]  justify-center items-center font-bold border border-[#FFB432]/50 hover:opacity-80'>
                                    ...
                                    {betAutoState[i] === "auto" ?
                                        <div className='flex justify-between gap-2 items-center min-w-[300px]'>
                                            <button
                                                ...
                                                }} className={`flex w-[72px] lg:w-[96px] h-[24px] 3xl:w-[212px] 3xl:h-[48px] rounded-full ${aviatorState.RemainedAutoPlayCount[i] > -1 ? "bg-red-700" : "bg-gradient-to-b from-[#07BDE5] to-[#07BDE588]"}   justify-center items-center font-bold border border-[#9FEEFF] text-[10px] lg:text-[12px] uppercase`}>
                                                {
                                                    aviatorState.RemainedAutoPlayCount[i] > -1 ?
                                            ...
                                            <div className='flex gap-1 items-center'>
                                                ...
                                                <SwitchButton
                                                    disabled={betPlaceStatus[i] === "success"}
                                                ...
                                                <div className='flex gap-1 px-3 py-[2px] rounded-full bg-[#1F1F1F]'>
                                                    <input disabled={!enabledAutoCashOut} readOnly={betPlaceStatus[i] === "success"} type="text" value={autoCashVal[i]}
                                                        onChange={(e) => {
            ...
            <ChatBoard className={`${chatOpen ? "flex" : "hidden"}`} />
        </div >
    )
}
